...
*  备    注：
*************************************************************************/
void DMA_PORTx2BUFF_Init(DMA_CHn CHn, void *SADDR, void *DADDR, PTXn_e ptxn, DMA_BYTEn byten, uint32 count, DMA_PORTx2BUFF_cfg cfg)
{
    uint8 n, i, tmp;

    ASSERT(                                             //用断言检测 源地址和每次传输字节数是否正确
        (   (byten == DMA_BYTE1)                    //传输一个字节
            && ( (SADDR >= &PT_BYTE(A,0,PDIR)) && (SADDR <= ( &PT_BYTE(E,3,PDIR) )))
        )

        || (   (byten == DMA_BYTE2)                   //传输两个字节(注意，不能跨端口)
               && ( (SADDR >= &PT_BYTE(A,0,PDIR))
                    && (SADDR <= ( &PT_WORD(E,1,PDIR) ))
                    && (((uint32)SADDR & 0x03) != 0x03) )         //保证不跨端口
           )

        || (   (byten == DMA_BYTE4)                   //传输四个字节
               && ((SADDR >= &PT_BYTE(A,0,PDIR)) && (SADDR <= ( &PT_BYTE(E,0,PDIR) )))
    ...
    /*  输入源管脚选择功能脚  */
    n = (uint8)(((uint32)SADDR - ((uint32)(&PT_BYTE(A,0,PDIR)))) & 0x3f);         //最小的引脚号
    tmp = n + (BYTEs << 3);                                         //最大的引脚号
    for(i = n; i < tmp; i++)
    {
        PORT_PCR_REG(PORTX[   ((((uint32)SADDR)&0x1ff)>>6)    ], i) = (0
                | PORT_PCR_MUX(1)
                | PULLDOWN             //输入源应该下拉，默认读取到的是0
                                                                   );
    }
...
*  备    注：
*************************************************************************/
void DMA_count_Init(DMA_CHn CHn, PTXn_e ptxn, uint32 count, DMA_Count_cfg cfg)
{
